...
?>
	...
	<link rel="stylesheet" href="css/looma-library.css">
	</head>
...
<?php
		...
		foreach (new DirectoryIterator($path) as $fileInfo) {
    		...
    		if (($fileInfo -> isDir()) && $file[0] !== "." && ( ! file_exists($path . $file . "/hidden.txt")))  
				...
				if (file_exists($path . $file . "/images.txt")) {
					echo "<td><a href='looma-picture-player.php?dir=$path$file'><button class='col-md-2 activity zero-scroll img with-play'><img src='images/play-slideshow-icon.png' class='img-responsive'></button></a><a href='looma-library.php?fp=" . $path . $file . "/'><button class='activity img zeroScroll beside-play'>" . thumb_image($path .$file) . $file . "</button></a></td>";
				}
				else {
					echo "<td><a href='looma-library.php?fp=" . $path . $file . "/'><button class='activity img zeroScroll'>" . thumb_image($path .$file) . $file . "</button></a></td>";
				}
	...
	foreach (new DirectoryIterator($path) as $fileInfo) {
		...
		if (($file[0]  == ".") || strpos($file, "_thumb") || $file == "thumbnail.png" || $file == "images.txt") continue;
