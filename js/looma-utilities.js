        ...
        switch (button.getAttribute("data-ft")){
            ...
            case "epaath":
                ...
                */
                ...
                break;
            case "pp":
                window.location = 'looma-picture-player.php?dir=' + encodeURIComponent(button.getAttribute('data-fp')) + "&source=activities";
                break;
            default:
...
//LOOMA POPUP FUNCTIONS BEGINS HERE

/**
 * This function creates a popup message box that can be dismissed by the user.
 * @param message - The message the user is presented.
 * */
LOOMA.notice = function(message){
    $('.popup').remove();
    $(document.body).append("<div class= 'popup clicked-dismissal'><p>" + message +
        "</p><button id ='close-popup' class ='popup-button'>OK</button></div>");
    fadeInPopup();
...
/**
 * Very similar to clickedDismissal, but is dismissed automatically after a given period of time.
 * @param message - The message the user is presented.
 * @param time - The time in milliseconds the popup takes to dismiss automatically.
 * */
var popupInterval;
LOOMA.timeDismissal = function(message, time) {
    $('.popup').remove();
    $(document.body).append("<div class='popup timed'><p>" + message +
        "</p><button id='close-popup' class='popup-button'></button></div>");
    fadeInPopup();
    var timeLeft = time;
    var popupButton = $('.popup-button');
    popupButton.text("OK (" + Math.round(timeLeft/1000) + ")");
 ...
 * @param message - The message the user is presented in question format.
 * @param callback - A function where the user's response (true or false) will be sent.
 * */
LOOMA.confirmation = function(message, confirmed, canceled) {
    $('.popup').remove();
    $(document.body).append("<div class='popup confirmation'><p>" + message +
        "</p><button id='cancel-popup' class='popup-button'>cancel</button>" +
        "<button id='confirm-popup' class='popup-button'>confirm</button></div>");
    fadeInPopup();
    console.log("test");
 ...
 * @param message - The message the user is presented, prompting them to enter text.
 * @param callback - A function where the user's text response will be sent.
 * */
LOOMA.textEntry = function(message, callback) {
    $('.popup').remove();
    $(document.body).append("<div class='popup textEntry'><p>" + message +
        "</p><textarea id='popup-textarea'></textarea>" +
        "<button id='confirm-popup' class='popup-button'>OK</button></div>");
    fadeInPopup();
    $('#confirm-popup').click(function() {
